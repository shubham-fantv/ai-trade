  ...
  return (
    <div className="bg-[#FF0000]  text-white pt-16 sm:pt-24 mx-auto bg-gradient-to-r from-[#E7DCFF] to-[#D8EDFF] ">
      ...
      <div>
        ...
        <h4 className=" text-[#706383] mb-6 pt-2 text-lg sm:text-[22px] font-semibold  text-center px-4 leading-[30px]">
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-4 my-10 mx-auto px-4">
        ...
        >
          ...
          <span className="ml-10 md:ml-12 text-sm md:text-base text-center">
        ...
        <button
          onClick={() => alert("coming soon")}
          className="w-full md:w-auto px-6 py-3 md:py-4 text-black bg-white border border-gray-300 rounded-full shadow-md hover:bg-gray-100 text-sm md:text-base"
        >
          Hire Agent Now
        </button>
      </div>
      ...
      <div className="relative overflow-hidden  p-4 pb-8 min-h-[500px] bg-custom-gradient">
        <div className="relative z-10">
          {!isMobile && (
            <div className="grid text-[#302249] grid-cols-8 text-sm mb-2 px-4 min-w-[1100px] overflow-x-auto">
          ...
          {isMobile ? (
            <div className="space-y-3">
              {filteredResults?.map((agent) => (
                <div key={agent._id} className="rounded-lg bg-black/90 border border-white/10">
                  <div
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1 text-sm font-semibold">
                          {agent.name}
                          <span className="text-xs text-[#D2D2D2]">({agent.ticker})</span>
                          <svg
                            width="20"
                            height="20"
                            viewBox="0 0 20 20"
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-sm text-[#CCFF00]`}>
                        <p className="text-xs text-[#D2D2D2]">(M Cap)</p>
                    ...
                    <div className="p-3 pt-0 border-t border-white/10">
                      <div className="space-y-2 text-sm">
                        <div className="flex pt-3 justify-between">
                          <span className="text-[#D2D2D2]">Category:</span>
                          <span>{agent.category}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-[#D2D2D2]">Market Cap:</span>
                          <span>${agent.marketCap}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-[#D2D2D2]">TVL:</span>
                          <span>${agent.tvl}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-[#D2D2D2]">Holders:</span>
                          <span>{agent.holders}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-[#D2D2D2]">24H Vol:</span>
                          <span>${agent.volume24}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-[#D2D2D2]">24H Change:</span>
                          <div
                      ...
                      <a
                        ...
                        className="w-full flex pt-2 text-sm underline btn btn-primary text-[#CCFF00]"
                      >
                        View Details
                      </a>
                    </div>
            ...
            <div className="rounded-xl overflow-x-auto  bg-[#EEEDF166] backdrop-blur-sm text-sm">
                    ...
                    >
                      <div className="flex items-center justify-start col-span-2">
                        <div className="flex items-center gap-3">
                          <img
                            src={agent.profilePic}
                            className="h-[48px] w-[48px] rounded-[8px] flex items-center justify-center"
                            alt={"Token_Icon"}
                          />
                          <div>
                            <div className="text-[14px] text-[#302249] font-normal flex items-center">
                              {agent?.name}
                              <span
                                style={{ fontSize: "12px" }}
                                className="text-[12px text-[#302249] px-1.5 rounded bg-black/20 "
                              >
                                ({agent?.ticker})
                              </span>
                              <svg
                            ...
                            <div className="w-[150px] mt-1 px-2 text-[10px] text-[#CCFF00] bg-[#353B1A] flex justify-between items-center gap-1 border-[2px] border-[#CCFF00]/30 rounded-[8px]">
                              {formatWalletAddress(agent?.address)}
                              <svg
                      ...
                      <div
                        className={`text-center text-[#302249] text-[14px] font-normal leading-[21px]`}
                      >
                        &nbsp; {agent.category}
                      </div>
                      <div
                      ...
                      <div
                        className={`text-center text-[14px] font-normal leading-[21px] whitespace-nowrap ${
                          agent?.change24?.text?.startsWith("+")
                            ? "text-[#26C281]"
                            : "text-[#ED1C24]"
                        }`}
                      ...
                      <div className="text-center text-[#302249] text-[14px] font-normal leading-[21px]">
                        {agent.tvl}
                      </div>
                      <div className="text-center text-[#302249] text-[14px] font-normal leading-[21px]">
                        {agent.holders}
