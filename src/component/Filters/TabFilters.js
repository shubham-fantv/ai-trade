...
const FilterTabs = ({ activeTab, setActiveTab, isMobile }) => {
  ...
  return (
    <div
      className="flex gap-6 p-2 overflow-x-auto items-center rounded-full  "
      style={
        {
    ...
    >
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => handleTabClick(tab.id)}
          className={`px-5 py-2 rounded-full text-base text-[#302249]  transition-all flex-shrink-0
            ${
              activeTab === tab.id
                ? "bg-[#302249] text-[#FFFFFF] shadow-inner font-bold"
                : "hover:bg-white/10 backdrop-blur-3xl font-medium"
            }`}
          style={{
            backdropFilter: activeTab != tab.id ? " blur(44px)" : "",
            boxShadow: activeTab != tab.id ? "0px 0px 8px 0px #00000026" : "",
          }}
        >
          {tab.label}
        </button>
